...
//$('.gogoro-select').on('click', function () {
...
/*
|--------------------------------------------------------------------------
| Basic select wrap directive
|--------------------------------------------------------------------------
|
*/
gogoroApp.directive('gogoroSelect', function () {
    return {
        restrict: "A",
        scope: {},
        replace: false,
...
/*
|--------------------------------------------------------------------------
| select for address with county, city district, zip code
|--------------------------------------------------------------------------

Html usage:

    <!--國家-->
    <span class="gogoro-select gogoro-select--xs-100" gogoro-select>
        <select-country selected-value="country1"
                        input-hidden-name="contact1-country">
        </select-country>
    </span>

    <!--縣市-->
    <span class="gogoro-select gogoro-select--xs-100" gogoro-select>
        <select-city selected-value="city1"
                        input-hidden-name="contact1-city"
                        district-class-name="contact1-distirct">
        </select-city>
    </span>

    <!--地區-->
    <span class="gogoro-select gogoro-select--xs-100" gogoro-select>
        <select-district selected-value="distirct1"
                            input-hidden-name="contact1-distirct"
                            district-class-name="contact1-distirct"
                            zip-class-name="contact1-zip-code">
        </select-district>
    </span>

    <!--ZipCode-->
    <div class="form-group select-block">
        <label class="block" for="ZipCode"><strong>@*區碼*@</strong> <span class="optional">(自動産生)</span></label>
        <input type="text" class="form-control contact1-zip-code" id="ZipCode" readonly placeholder="區碼">
    </div>

    <!--聯絡地址-->
    <input type="text" class="form-control" name="Address" ng-mode="user.Address" placeholder="請輸入住家地址">


*/
/* selectAddressData service*/
gogoroApp.factory('selectAddressData', ["$rootScope", function ($rootScope) {
    // debuging 
    var debug = true;

    var log = function (subject, val) {
        if (debug) {
    ...
    getCityById = function (id) {
        ...
        for (var i = 0; i < cities.length; i++) {
            //console.log('objs[i].Id', objs[i].Id);
            if (cities[i].Id == id) {
                result = cities[i];
            }
        }
    ...
    // add selected city data with district class name as key
    addSelectedCity = function (districtClassName, city) {
        cityCollection[districtClassName] = city;
    }

    // get district by district class name
    getDistrictListByDistrictClassName = function (districtClassName) {
        //console.log('cityCollection', cityCollection);
        return cityCollection[districtClassName].District;
    }
    ...
    return {
        ...
        addSelectedCity: addSelectedCity,
        getDistrictListByDistrictClassName: getDistrictListByDistrictClassName,
        log: log
...
/* select country */
gogoroApp.directive('selectCountry', ['selectAddressData', function (selectAddressData) {
    return {
        template: '<select ng-model="selectedValue" ng-change="onChange(selectedValue)" ng-options="c.id as c.name for c in countries"><option value="">請選擇</option></select>',
        restrict: 'E',
        replace: true,
        scope: {
            selectedValue: '='
        },
        controller: ["$scope", "selectAddressData", function ($scope, selectAddressData) {
            $scope.countries = selectAddressData.countries;
        }],
        link: function (scope, elem, attrs) {
           
            // create hidden input, and insert after this select
            elem.after(
                $("<input />").attr({ 'type': 'hidden', 'name': attrs.inputHiddenName, 'value': scope.selectedValue })
            );

            // get hidden input
            var $input = elem.next('input[type="hidden"]');

            // on change event
            scope.onChange = function (selectedValue) {
                scope.selectedValue = selectedValue;
                // update value for hidden input
                $input.val(scope.selectedValue);
            }
        }
    };
}]);
...
/* select city */
gogoroApp.directive('selectCity', ['selectAddressData', function (selectAddressData) {
    return {
        template: '<select ng-model="citySelectedValue" ng-change="onChange(citySelectedValue)" ng-options="city.Id as city.ChineseName for city in cities"><option value="">請選擇縣市</option></select>',
        restrict: 'E',
        replace: true,
        scope: {
            selectedValue: '='
        },
        controller: ["$scope", "$element", "selectAddressData", function ($scope, $element, selectAddressData) {

            var districtClassName = $element.attr('district-class-name');
            var usingVmCities = $element.attr('using-vm-cities');
            if (angular.isUndefined(usingVmCities)) {
                // get cities list from service
                $scope.cities = selectAddressData.cities;
            } else {
                // get cities list from service
                $scope.cities = selectAddressData.vmCities;
            }

            // get a city by id (reference)
            $scope.getCityById = selectAddressData.getCityById;

            // this is mainly used, when a user
            // press 'same as buyer' link
            $scope.$watch(function () {
                return $scope.selectedValue;
            }, function (newValue, oldValue) {

                if (angular.isDefined(newValue)) {
                    $scope.citySelectedValue = newValue;
                    // get a city by id
                    var city = $scope.getCityById(newValue);
                    //selectAddressData.log('city.onChanged Current city obj:', city);

                    // add selected city with key name of district class name
                    selectAddressData.addSelectedCity(districtClassName, city);
                    selectAddressData.getDistrictListByDistrictClassName(districtClassName);
                }
        ...
        link: function (scope, elem, attrs) {
            ...
            // district class name
            var districtClassName = attrs.districtClassName;
            ...
            if (angular.isDefined(scope.selectedValue)) {
                ...
                scope.citySelectedValue = scope.selectedValue;

                var city = scope.getCityById(scope.selectedValue);
                selectAddressData.addSelectedCity(districtClassName, city);
                selectAddressData.getDistrictListByDistrictClassName(districtClassName);
            ...
            scope.onChange = function (selectedValue) {
                ...
                // get a city by id
                var city = scope.getCityById(selectedValue);
                //selectAddressData.log('city.onChanged Current city obj:', city);
...
/* select district */
gogoroApp.directive('selectDistrict', ['selectAddressData', '$filter', function (selectAddressData, $filter) {
    return {
        template: '<select ng-model="districtSelectedValue" ng-change="onChange(districtSelectedValue)" ng-options="district.Id as district.ChineseName for district in districts"><option value="">請選擇地區</option></select>',
        restrict: 'E',
        replace: true,
        scope: {
            selectedValue: '=',
        ...
        controller: ["$scope", "$element", "selectAddressData", function ($scope, $element, selectAddressData) {
            $scope.districtClassName = $element.attr('district-class-name');
            $element.addClass($scope.districtClassName);

            // add selected city with key name of district class name
            selectAddressData.addSelectedCity($scope.districtClassName, []);

            // get district list from service
            $scope.districts = selectAddressData.getDistrictListByDistrictClassName($scope.districtClassName);

            // watch district data in service, 
            // and update $scope.districts value if service value changed 
            $scope.$watch(function () {
                return selectAddressData.getDistrictListByDistrictClassName($scope.districtClassName);
            }, function (newDistrict, oldDistrict) {
                
                if (angular.isDefined(newDistrict)) {
                    //console.log('xxx newDistrict', newDistrict);
                    $scope.districts = newDistrict;
                }
            ...
            $scope.$watch(function () {
                ...
                //console.log('');
                //console.log('-----'+$scope.districtClassName+'-----');
                //console.log('selectDistrict selectedValue(newValue)', newValue);
                //console.log('selectDistrict selectedValue(oldValue)', oldValue);
             
                $scope.districtSelectedValue = newValue;
        ...
        link: function (scope, elem, attrs) {
            ...
            // set default selected value 
            if (angular.isDefined(scope.selectedValue)) {
                console.log('selectDistrict scope.selectedCityValue', scope.selectedCityValue);
                console.log('selectDistrict scope.selectedValue', scope.selectedValue);

                //
                var city = selectAddressData.getCityById(scope.selectedCityValue);
                ...
                scope.districtSelectedValue = scope.selectedValue;
            }
            ...
            scope.onChange = function (selectedValue) {
                ...
                //selectAddressData.log('districts list :', scope.districts);
                //selectAddressData.log('district.onChanged district.Id:', selectedValue);

                if (selectedValue != null) {
                    var zipCode = $filter('filter')(scope.districts, { 'Id': selectedValue })[0].ZipCode;
                    $('.' + attrs.zipClassName).val(zipCode);
                    //selectAddressData.log('zipCode', zipCode);
                }
